 ...
 */
const checkIfPathExists = (userPath) => {
 ...
 */
const getPathsOfMdFiles = (userPath, mdFiles = []) => {
  ...
  if (!dirOrFile) {
    if (checkFileExtension(userPath)) {
      ...
      throw new Error('Path is not a directory or .md file');
    }
  } else {
 ...
 */
const responseHTTPRequest = (linkInformationObject) => {
  return new Promise((resolve, reject) => {
...
const prueba = {
  href: 'https://www.figma.com/file/Q12DjjRbnFTJiNU7yWEPSA/Social-Network-Laboratoria-%7C-Alta-Fidelidad?node-id=22%3A3',
  text: 'aquí',
  file: '/home/amappola/programming_CS/3webDevelopment/3laboratoria/1proyectosBootcamp/4.4markdownLinks/md-links/mdFiles/socialNetwork/readmeSocialNetwork.md'
}

// responseHTTPRequest(prueba);
  ...
  if (pathExists) {
    ...
    if (validate === false) {
      ...
      return new Promise((resolve, reject) => {
        ...
        Promise.all(mdLinksValidated).then((mdLinksResult) => {
          resolve(mdLinksResult.flat());
        })
      })
    }
