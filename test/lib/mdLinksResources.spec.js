...
import {
  checkIfPathExists, getAbsolutePath, checkDirOrFile, checkFileExtension, getPathsOfMdFiles, getLinks, getFileLinks, validateLink,
} from '../../lib/mdLinksResources';
import {
  pathsMock, singlePath, contentFile, nullContent, linksFile, getFilesLinksMock, linkToValidate, linkValidated, brokenLink, brokenLinkValidated,
} from '../manualMocks/mdLinksResourcesMocks.js';
...
describe('Tests for getFileLinks()', () => {
  it('should return a promise that resolves into an array of links', () => getFileLinks('test/manualMocks/mdFilesMock/file.md').then((result) => {
    expect(result).toEqual(getFilesLinksMock);
  }));
});

describe('Tests for validateLinks()', () => {
  it('should return an object with the information of the response of the HTTP request', () => validateLink(linkToValidate).then((result) => {
    expect(result).toEqual(linkValidated);
  }));

  it('should return an object with the information of the response of the HTTP request and an error when the link is broken', () => validateLink(brokenLink).then((result) => {
